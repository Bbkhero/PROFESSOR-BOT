...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton("Add Me To You Chat", url=f"http://t.me/{temp.U_NAME}?startgroup=true")
            ],[
...
async def settings(client, message):
    ...
    if settings is not None:
        buttons = [[
            ...
            InlineKeyboardButton(f"Distracted Content: {'On' if settings['file_secure'] else 'Off'}", f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}')
            ],[
        ...
        await message.reply_text(
            text=f"<b>Change Your Setting For{title} As Your Wish</b>",
...
async def save_template(client, message):
    ...
    if chat_type == enums.ChatType.PRIVATE:
        ...
        if grpid is not None:
            ...
            try:
                ...
                return await message.reply_text("Make Sure Im Present In Your Group!!", quote=True)
        else:
            return await message.reply_text("Im Not Connected To Amy Group!", quote=True)
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        grp_id = message.chat.id
        title = message.chat.title
    else: return
    ...
    await sts.edit(f"Successfully Changed Template {title} Tᴏ\n\n{template}")
...
async def geg_template(client, message):
    ...
    await sts.edit(f"Current Template For{title} Is\n\n{template}")
